...
// PopCountClearRightmost returns the population count (number of non-zero bits) of x, by repeatedly turning off the
// rightmost bit until none remain.
func PopCountClearRightmost(x uint64) (sum int) {
	for x > 0 {
		x = x & (x - 1)
		sum++
	}
