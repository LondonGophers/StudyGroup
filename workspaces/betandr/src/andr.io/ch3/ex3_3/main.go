...
func main() {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			if z > 0 {
				fmt.Printf("<polygon points='%g,%g %g,%g %g,%g %g,%g' style='fill:black;stroke:red;stroke-width:1'/>\n",
					ax, ay, bx, by, cx, cy, dx, dy)
			} else {
				fmt.Printf("<polygon points='%g,%g %g,%g %g,%g %g,%g' style='fill:black;stroke:blue;stroke-width:1'/>\n",
					ax, ay, bx, by, cx, cy, dx, dy)
